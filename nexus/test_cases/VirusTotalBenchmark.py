...
METADATA = {
    "subset": "VirusTotalBenchmark",
    "description": "Test cases for VirusTotalBenchmark API calls"
}
...
def vt_get_object_descriptors_related_to_ip_address(ip, relationship, x_apikey, limit=None, cursor=None):
    ...
    for key, value in args_dict.items():
        if value:
            return_dict[key] = value
    correctness.append({"vt_get_object_descriptors_related_to_ip_address": locals()})
...
def vt_get_object_descriptors_related_to_domain(
    ...
    for key, value in args_dict.items():
        if value:
            return_dict[key] = value
    correctness.append({"vt_get_object_descriptors_related_to_domain": locals()})
